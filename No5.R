...
##set path
setwd("D:/Recommendation_R_Exercise/1M")
...
evaluateModels<-function(movieRatingMat){
  ...
  scheme <- evaluationScheme(movieRatingMat, method="cross-validation", train = .9,
...
##run "IBCF" recommender
rec_model<-createModel(movieRatingMat,"IBCF")
userID<-5
topN<-5
recommendations(movieRatingMat,rec_model,userID,topN)
